...
dt = 0.1  # [s] time tick
WB = 18.57  # [m] wheel base of vehicle
set_point = 10.0  # This is the set point in m/s
...
class Robot:
    ...
    def __init__(self, pose=[0.0, 0.0, 0.0], velocity=0.0, angular_velocity=0.0, wheel_speed=[0.0, 0.0]):
        ...
        self.wheel_span = 18.7  # cm
        self.max_wheel_speed = 2 * math.pi * 1.1136  # rad/s
        self.max_angular_velocity = 2 * self.max_wheel_speed * self.wheel_radius / self.wheel_span  # rad/s
        self.max_velocity = (self.max_wheel_speed * self.wheel_radius) / 100.0  # cm/s
        self.max_acceleration = self.max_velocity / 1.0  # m/s^2

        self.max_encoder_frequency = [1000.0, 1000.0]
        self.encoder_frequency = [0.0, 0.0]
        self.left_encoder_frequency_hist = [self.encoder_frequency[0]]
        self.right_encoder_frequency_hist = [self.encoder_frequency[1]]

        self.pose = pose  # [x [m], y [m], theta [radians]]
    ...
    def wheel_speed_2_encoder_freq(self):
        self.encoder_frequency[0] = self.wheel_speed[0] * 74.83 * 12.0
        self.encoder_frequency[1] = self.wheel_speed[1] * 74.83 * 12.0

        self.left_encoder_frequency_hist = np.append(self.left_encoder_frequency_hist, [self.encoder_frequency[0]],
                                                     axis=0)
        self.right_encoder_frequency_hist = np.append(self.right_encoder_frequency_hist, [self.encoder_frequency[1]],
                                                     axis=0)
    ...
    def control_motors(self, velocity, turn_radius):
        # Calculate velocity and angular velocity
        self.velocity = velocity * 100.0
        np.append(self.velocity_history, [velocity], axis=0)
...
def plot_arrow(pose, length=1.0, width=0.5, fc="r", ec="k"):
    """
    Plot arrow
    """

    if not isinstance(pose[0], float):
        for ix, iy, iyaw in zip(pose[0], pose[1], pose[2]):
            plot_arrow(ix, iy, iyaw)
    else:
        plt.arrow(pose[0], pose[1], length * math.cos(pose[2]), length * math.sin(pose[2]),
...
def main():
    ...
    while simulation_time >= current_time and last_index > target_index:
        ...
        robot.control_motors(robot.max_velocity, turn_radius)
        robot.update_pose()
        ...
        if show_animation:
            ...
            plt.axis("equal")
            plt.grid(True)
            plt.title("Speed[m/s]:" + str(0.24)[:4])
            plt.pause(0.001)

    assert last_index >= target_index
    if show_animation:  # pragma: no cover
        plt.subplots(1)
        plt.plot(path_taken.time, robot.left_encoder_frequency_hist, "-r", label="left wheel speed")
        plt.plot(path_taken.time, robot.right_encoder_frequency_hist, "-b", label="right wheel speed")
        plt.legend()
        plt.axis("tight")
        plt.grid(True)
        plt.show()
